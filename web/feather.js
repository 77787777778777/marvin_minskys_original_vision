...
class FeatherInterp {
  constructor(id) {
    ...
    // Shared TextEncoder + memoized UTF-8 byte cache (see utf8Bytes below).
    // Keyed on string value; cleared with the scratch arena per top-level eval.
    this._encoder = new TextEncoder();
    this._utf8Cache = new Map();
  }
  ...
  resetScratch() {
    ...
    // Byte cache is keyed on string values that only live within one eval's
    // scratch epoch; drop it too so it cannot grow without bound.
    this._utf8Cache.clear();
  }
  ...
  // Memoized UTF-8 encoding, stored ON the scratch object. The C engine
  // probes string bytes one at a time (string index/first/last, lsearch
  // scans); each probe used to re-encode the whole string -- O(n) per byte
  // access, O(n^2) per scan. Keying on the value via Map.get would still be
  // O(n) per probe (full-key hash), so the cache rides on the object record,
  // which the handle lookup reaches in O(1). Scratch objects are discarded
  // at eval boundaries, so the cache cannot go stale or grow unbounded.
  utf8Bytes(obj) {
    if (typeof obj === 'object' && obj !== null && obj.type === 'string') {
      let b = obj.utf8;
      if (b === undefined) {
        b = this._encoder.encode(obj.value);
        obj.utf8 = b;
      }
      return b;
    }
    return this._encoder.encode(this.getString(obj));
  }
...
async function createFeather(wasmSource) {
  ...
  const writeString = (str) => {
    ...
    let ptr = wasmInstance.exports.alloc(bytes.length + 1);
    if (ptr + bytes.length + 1 > wasmMemory.buffer.byteLength) {
      // Heap exhausted: grow linear memory and retry the allocation.
      // (Old linear addresses stay valid across grow; only the JS-side
      // ArrayBuffer is detached, which is why we re-read wasmMemory.buffer.)
      const extraPages = Math.ceil((bytes.length + 2) / 65536) + 8;
      wasmMemory.grow(extraPages);
      ptr = wasmInstance.exports.alloc(bytes.length + 1);
    }
